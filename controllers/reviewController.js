...
/**
 * Sukuria naują atsiliepimą ir atnaujina susijusio objekto (destination/hotel/agency) įvertinimą
 */
const createReview = async (req, res) => {
  try {
    const { rating, comment, destination, hotel, agency } = req.body

    // Patikrinamas reitingo galiojimas
    if (!rating || rating < 1 || rating > 5) {
      return res.status(400).json({ message: 'Rating must be between 1 and 5' })
    }

    // Sukuriamas naujas atsiliepimas
    ...
    // Apskaičiuojamas naujas vidutinis reitingas
    const filter = {}
    ...
    // Įrašomas naujas vidutinis reitingas į atitinkamą modelį
    if (destination) {
...
/**
 * Grąžina atsiliepimus pagal filtrą (destination / hotel / agency)
 */
...
/**
 * Ištrina atsiliepimą (leidžiama tik autoriui arba adminui)
 */
...
/**
 * Atnaujina esamą atsiliepimą (leidžiama tik autoriui arba adminui)
 */
...
/**
 * Grąžina visus sistemos atsiliepimus (naudojama administravimui)
 */
